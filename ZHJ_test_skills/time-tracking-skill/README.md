...
每次保存先实时通过 MySQL 花名册硬校验员工在职及所选业务线归属；通过后数据写入本地 JSONL，再由定时任务同步到团队共享 MySQL 数据库，
...
直接维护 MySQL `agent_team_roster` 表的团队成员；一名员工属于多条业务线时写多行：

```sql
INSERT INTO agent_team_roster (biz_line, name, role, active) VALUES
  ('智慧记+运营系统', '詹惠英', '功能测试', 1),
  ('智慧记零售', '詹惠英', '功能测试', 1);
```

> 表中没有 `biz_line_code` 字段。`load_roster.py` 读取 `biz_line` 后才派生编码；`AILIT` 只写入 `agent_time_tracking.biz_line_code`。
...
- 泛化改动：`biz_line` 全部可配置、花名册按姓名聚合多行中文 `biz_line` 并在脚本输出中派生 `biz_line_code`、报告标题/表名动态化
