# 时间节省追踪规则（v5.9 — 强制反馈 + 反馈后直接保存 + 参考时间 + MySQL 共享数据库同步 + AI 自动初始化 + 定时任务自动注册 + 花名册实时查 MySQL + **立即触发+阻塞下一步** + 智能体执行耗时采集 + 定时任务注册去占位符(register_sync_tasks.py) | 智慧记运营多业务线版）

> **智慧记运营套件优先规则**：执行本套件时必须先读取 `references/zhj-eight-stage-workflow.md`。它将追踪点扩展为⓪、①、②、③、④、⑤、⑦、⑧；③与④仅凭当前会话的同一 `session_id` 累计为一条“生成用例（06）”记录，③后提醒测试人员留在当前会话继续④；员工反馈后直接保存，**不进行二次确认**；跨业务线员工按编号选择本次业务线。保存脚本会硬校验员工在职及所选中文业务线归属，失败时不得绕过并应提示联系管理员。若本文件旧内容有冲突，以该文件为准。

> **⛔ v5.5 核心强化（强制 + 立即 + 阻塞，不可绕过）**：
> 八阶段中的每一步产出文件交付给用户后，**必须**按以下固定顺序执行，**顺序不可调换、不可跳过**：
> 1. **完成通报 + 产出物展示**
> 2. **立即触发时间数据收集**（详见本文件第四节）—— **禁止先说"下一步怎么走"或展示"进入② / 进入④"等选项**
> 3. **解析后直接保存，不得二次确认**（`scripts/record_time_saved.py`）
> 4. **确认记录完成**后，方可展示下一步选项或等待用户指令
>
> 身份及业务线硬校验通过后，用户拒绝填写时**最多追问 2 次**，仍拒绝则记录 `time_saved_hours=0` 并标注 `"用户未反馈"` ——但**仍属于"完成时间收集环节"**，之后才允许展示下一步选项进入下一步骤。校验失败不得用 0 小时绕过。
>
> **⛔ 禁止行为**：❌ 在收齐时间数据前展示"下一步选项" / ❌ 在收齐前进入下一步骤 / ❌ 自作主张跳过时间询问 / ❌ 把时间询问合并到下一步骤的对话里。
>
> **v5.5 多业务线共享 bat（2026-08-21）**：sync_task.bat 由「按业务线写死 set BIZ_LINE=」升级为「接收第 1 个参数 %1 决定业务线」，定时任务注册命令 /tr 末尾传入 {biz_line}，同一份 bat 复用服务多业务线（效贷/泾渭云/效融/小贷/智慧记+运营系统/AI进销存/智慧记零售/国际版Ailit），彻底去掉硬编码。配套效贷测试专家 agent v1.6.3 在业务线确定后强制校验 time-tracking/{biz_line}/mysql_config.json 真实生成才算流程闭环，修复「扫描任一份即可」导致非首条业务线（如效融）配置被跳过、数据无法入库的漏洞。v5.4 的「立即+阻塞」措辞保持不变。
...
> - 身份及业务线实时 MySQL 硬校验通过后，时间数据**只写本地 JSONL**，再由**定时任务（每日 09:00 / 12:00 / 18:00）**
...
1. **调用花名册查询脚本，实时查询 MySQL `agent_team_roster` 表**（机器可读 JSON，**禁止读本地 `team_roster.yaml`**；后者已弃用，人员由管理员直接维护在 MySQL agent_team_roster 表）：
   ...
   数据库查询字段只有 `biz_line, name, role, active`。脚本按姓名聚合多行 `biz_line`，并通过 `biz_line_helper.py` 派生输出中的 `members[*].biz_line_code`；该数组不是数据库字段。输出示例：
...
4. **匹配成功** → 确定本次会话的业务线 `{biz_line}`：
   ...
   - 若聚合后的 `biz_line` 只有一条 → 直接使用该业务线
   - 若同一员工在表中有多行业务线记录 → **按聚合后的中文 `biz_line` 列出编号选项让用户输入数字选择**，禁止自由文本回答（避免笼统回答导致匹配不准确）：
     ```
     👤 {姓名}，你好！你属于以下多条业务线，请选择本次处理哪条（输入数字即可）：
        ...
        3. 智慧记零售
     ```
     - 编号按脚本聚合得到的 `biz_line` 顺序生成；对应编码由 `biz_line_to_code` 派生（包括“智慧记+运营系统”→`ZHJ`、“AI进销存”→`AIJXC`、“智慧记零售”→`ZHJLS`、“国际版Ailit”→`AILIT`）
...
### 第 3 步：解析后直接进入保存（v5.9，不得二次确认）

解析出时间数据后直接执行第 4 步调用脚本保存，不再展示确认卡片，也不等待“确定/确认”等回复。用户主动纠正数据时，以最新反馈重新保存；本规则不能覆盖身份及业务线硬校验。

### 第 4 步：写入本地 JSONL（兜底，v5 主流程）

员工反馈解析完成后，立即调用 `record_time_saved.py` 脚本写入本地 JSONL：
...
> 智慧记运营套件必须显式传入身份确认环节选定的 `--biz-line`，不得依赖默认值猜测。
> 如果用户用的是人天，则用 `--person-days {人天数}` 代替 `--hours`。脚本内部自动换算为小时存储。

**记录的业务数据只写本地，但保存前必须实时查询 MySQL 花名册；查询或校验失败时拒写并提示联系管理员**。业务时间数据的 MySQL 同步由定时任务完成
（见第六节），**AI 在记录环节不需要、也不应该直接调用 `sync_to_mysql.py`**。

### 第 5 步：确认记录

脚本保存成功后，只给出关键信息，不要解释调用脚本、存储文件、同步机制等技术细节：
...
- 业务时间数据先写本地；保存前的身份及业务线校验依赖 MySQL，失败时拒写；
...
| 模式 | 说明 | 配置值 |
|------|------|--------|
| 本地 + MySQL（推荐） | 硬校验通过后写 JSONL + 定时任务同步共享 MySQL | `mysql` |
| 仅本地 | 仅改变时间数据同步方式；身份及业务线仍实时查 MySQL | `local` |
...
2. **反馈后直接保存（v5.9）**：解析出时间数据后立即调用脚本，不展示二次确认，也不等待用户回复“确定”。
...
12. **MySQL 初始化自动完成（v1.5.1）+ 花名册实时查 MySQL（v1.5.2）**：会话启动检查到 `mysql_config.json` 缺失时，必须按第一节第 3 点**自动完成初始化**（索要密码 → 自动调用 `init_mysql_config.py` → 解析 JSON 结果），**禁止要求测试人员手动打开 CMD**；配置完成前可以继续不涉及时间落盘的工作，但任何时间记录都必须硬拦截。用户拒绝提供密码时说明无法完成身份校验和时间记录，并提示联系管理员。身份识别一律实时查询 MySQL `agent_team_roster` 表（调用 `scripts/load_roster.py --json`），**禁止读 `team_roster.yaml` 做身份匹配**。
