# 时间节省追踪规则（v5.10 — 完整步骤映射 + ③④按会话可独立/合并 + 强制反馈 + 反馈后直接保存 + MySQL 共享数据库同步 | 智慧记运营多业务线版）

> **智慧记运营套件优先规则**：执行本套件时必须先读取 `references/zhj-eight-stage-workflow.md`。它将追踪点扩展为⓪、①、②、③、④、⑤、⑦、⑧；③与④都以“生成用例（06）”记录，只有同一会话两步均完成时才凭相同 `session_id` 合并，否则各自独立保存，禁止跨会话合并；③后仍提醒测试人员留在当前会话继续④。员工反馈后直接保存，**不进行二次确认**；跨业务线员工按编号选择本次业务线。保存脚本会硬校验员工在职及所选中文业务线归属，失败时不得绕过并应提示联系管理员。若本文件旧内容有冲突，以该文件为准。
