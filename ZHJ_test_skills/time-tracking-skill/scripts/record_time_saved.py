...
# 参考时间表
REFERENCE_TIMES = {
    "00": {"min": 0.5, "max": 1.0, "unit": "小时", "basis": "按页面数量和附件数量浮动"},
    "01": {"min": 2.0, "max": 4.0, "unit": "小时", "basis": "按文档数量浮动，5个以上取上限"},
    "02": {"min": 2.0, "max": 3.0, "unit": "小时", "basis": "6维度评审（完整性/一致性/边界/异常/优先级/可测性）"},
    "05": {"min": 1.0, "max": 2.0, "unit": "小时", "basis": "按 AI 与人工用例差异数量浮动"},
    "06": {"min": 5.0, "max": 10.0, "unit": "小时", "basis": "详细用例与冒烟用例合计，按用例数量浮动"},
    "07": {"min": 1.0, "max": 2.0, "unit": "小时", "basis": "总结+差异对比+精华提炼+归档"},
    "08": {"min": 0.5, "max": 1.0, "unit": "小时", "basis": "按上传文件数量与 SVN 路径准备工作浮动"},
}
...
def merge_latest_record(records_path: str, incoming: dict) -> dict | None:
    ...
    total_hours = round(float(existing.get("time_saved_hours", 0)) + float(incoming["time_saved_hours"]), 2)
    existing["time_saved_hours"] = total_hours
    existing["time_saved_pd"] = round(total_hours / HOURS_PER_PD, 2)
    existing["total_hours"] = total_hours
    existing["agent_end_time"] = incoming.get("agent_end_time", "") or existing.get("agent_end_time", "")
    if incoming.get("agent_duration_minutes") is not None:
        existing["agent_duration_minutes"] = round(float(existing.get("agent_duration_minutes") or 0) + float(incoming["agent_duration_minutes"]), 2)
    if incoming.get("remark"):
        existing["remark"] = "; ".join(filter(None, [existing.get("remark", ""), incoming["remark"]]))
...
def main():
    ...
    parser.add_argument("--step-code", default="", help="步骤代码（00/01/02/05/06/07/08）")
