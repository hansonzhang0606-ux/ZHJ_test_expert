...
| ④ 生成冒烟用例 | `--step "生成用例" --step-code 06 --session-id <当前会话标识> --merge-existing` | 同会话有③则累计到③；没有③则将④独立保存为一条 `06` 记录 |
...
执行③或④前都必须生成或复用当前对话唯一的 `session_id`（例如 `ZHJ-20260829T142500-a1b2c3d4`）。同一会话依次完成③和④时原样复用该值，合并为一条 `06` 记录；只有③或只有④时，各自保存为一条 `06` 记录；不同会话的记录绝不合并。③记录成功后必须提示：“请在当前会话继续完成④生成冒烟用例；不要关闭或新建会话。”若在新会话直接执行④，应为该会话生成新标识并独立保存④，不能使用其他会话的标识。
