...
必须先通过 MySQL `agent_team_roster` 验证员工姓名；多业务线员工按编号选择本次业务线。保存脚本会再次硬校验“员工在职且所选中文业务线属于该员工”；任一条件不满足或花名册查询失败时不得保存，并提示联系管理员。不得将业务线固定为“智慧记+运营系统”。

按照 META 工作流执行⓪、①、②、③、④、⑤、⑦、⑧。只有身份及本次业务线硬校验通过后，才允许在每个阶段完成后立即收集工时并写入本地 JSONL；不得二次确认，也不得在记录完成前进入下一阶段。拒填时记录 0 小时也不得绕过身份或业务线校验。③与④必须在当前会话使用同一 `session_id`，累计为同一条“生成用例（06）”记录；③记录成功后提醒测试人员不要关闭或新建会话，继续完成④。
