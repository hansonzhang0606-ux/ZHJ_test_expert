...
按照 META 工作流执行⓪、①、②、③、④、⑤、⑦、⑧。只有身份及本次业务线硬校验通过后，才允许在每个阶段完成后立即收集工时并写入本地 JSONL；不得二次确认，也不得在记录完成前进入下一阶段。拒填时记录 0 小时也不得绕过身份或业务线校验。“生成用例（06）”必须使用显式 `session_id`：只有③时记录③，只有④时记录④；同一会话依次完成③和④时复用该标识并合并为一条记录，不得跨会话合并。③记录成功后仍须提醒测试人员留在当前会话继续④。
